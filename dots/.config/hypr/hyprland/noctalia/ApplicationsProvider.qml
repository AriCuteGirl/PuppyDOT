...
Item {
  ...
  function onOpened() {
    // Refresh desktop entries so apps installed after shell startup appear immediately.
    loadApplications();
  ...
  function getCleanLaunchCommand(app) {
    if (!app || !app.command)
      return [];

    const args = Array.isArray(app.command) ? app.command : (app.command.length !== undefined ? Array.from(app.command) : []);
  ...
  function createResultEntry(app, score) {
    return {
      ...
      "onActivate": function () {
        ...
        const cleanCommand = getCleanLaunchCommand(app);

        // Execute while this provider is still alive. Closing the launcher first can
        // destroy its QML context and invalidate this callback.
        if (!Settings.data.appLauncher.customLaunchPrefixEnabled && !app.runInTerminal && app.execute) {
          Logger.d("ApplicationsProvider", "Calling desktop entry execute() for: " + app.name);
          app.execute();
        ...
        if (Settings.data.appLauncher.customLaunchPrefixEnabled && Settings.data.appLauncher.customLaunchPrefix.trim() !== "") {
                         ...
                         if (app.runInTerminal && Settings.data.appLauncher.terminalCommand.trim() !== "") {
                           ...
                           Logger.d("ApplicationsProvider", "Executing cleaned command fallback: " + cleanCommand.join(" "));
                           Quickshell.execDetached(cleanCommand);
                         } else if (app.execute) {
